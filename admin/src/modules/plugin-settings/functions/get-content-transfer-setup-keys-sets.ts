...
export default (localizableTree: any[] = [], storedSetupSchema: any[] = []) => {
  ...
  let currentModelsSchemaComponentKeys = currentModelsSchemaKeys
    .filter((key: string) => key.match(regex))
    .map((key: string) => key.replace(regex, ''));
  let storedSetupSchemaComponentKeys = storedSetupSchemaKeys
    .filter((key: string) => key.match(regex))
    .map((key: string) => key.replace(regex, ''));
  ...
  const currentModelsSchemaComponentKeysComponentProp = currentModelsSchemaKeys.filter(
    (key: string) =>
      key.includes('__component__') && currentModelsSchemaComponentKeys.some((k: string) => key.includes(k))
  );
  const storedSetupSchemaComponentKeysComponentProp = storedSetupSchemaKeys.filter(
    (key: string) =>
      key.includes('__component__') && storedSetupSchemaComponentKeys.some((k: string) => key.includes(k))
  );
  ...
  let removedKeys = [];
  removedKeys.push(
    ...storedSetupSchemaComponentKeysComponentProp
      .map((key: string) => {
  ...
  let newKeys = [];
  newKeys.push(
    ...currentModelsSchemaComponentKeysComponentProp
      .map((key: string) => {
        const storedComponentName = get(storedSetupSchema, key);
        const currentComponentName = get(localizableTree, key);
   ...
   */
  const filteredStoredSetupSchemaKeys = storedSetupSchemaKeys.filter((key: string) => !removedKeys.includes(key));
