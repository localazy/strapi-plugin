...
const Overview: React.FC<OverviewProps> = ({ title, subtitle, isLoadingProp = false }) => {
      ...
      {!(isLoadingProp || isLoading) && (
        <Box paddingRight={10} paddingLeft={10}>
          ...
          >
            ...
            >
              <Grid.Item>
                <OverviewItem label={t('overview.connected_user')} value={identity.user.name} />
              </Grid.Item>
              <Grid.Item>
                <OverviewItem label={t('overview.connected_project')} value={identity.project.name} />
              </Grid.Item>
              <Grid.Item>
                <OverviewItem
                  label={t('overview.remaining_organization_keys')}
                  value={formatNumber(
                    (connectedProject?.organization?.availableKeys ?? 0) -
                      (connectedProject?.organization?.usedKeys ?? 0)
                  )}
                />
              </Grid.Item>
              <Grid.Item>
                <OverviewItemLink label={t('overview.open_project_in_localazy')} to={projectUrl} />
              </Grid.Item>
            </Grid.Root>
